...
func createPuzzle(parts []string) Puzzle {
	...
	for j := 1; j < len(parts)-1; j++ {
		...
		numbers := strings.SplitSeq(innerBrackets, ",")
	...
	jolts := strings.SplitSeq(innerBrackets, ",")
	for j := range jolts {
		i, _ := strconv.Atoi(j)
...
func readInput(_data string) []Puzzle {
	var puzzles []Puzzle
	lines := strings.SplitSeq(_data, "\n")
...
func main() {
	puzzles := readInput(data)
	minCounts := 0
	for _, puzzle := range puzzles {
		minCount := solve(puzzle.A, puzzle.b)
		minCounts += minCount
	}
	log.Println("PART 1::", minCounts)
